...
export const signUp = async (req, res) => {
  try {
    const {
      ...
      role, department, batch, profileUrl, requestMessage
    } = req.body;
    ...
    if (universityId) {
      ...
      if (!university) return res.status(400).send({ success: false, message: "University does not exist" });
    }
    ...
    const token = generateToken({ id: user.id, email: user.email, role: user.role, universityId: user.universityId });
...
export const signIn = async (req, res) => {
  try {
    ...
    if (!email || !password) return res.status(400).send({ success: false, message: "All fields are required" });

    const [user] = await db.select().from(users).where(eq(users.email, email)).limit(1);
    if (!user) return res.status(400).send({ success: false, message: "Invalid credentials" });

    const isValidPassword = await bcrypt.compare(password, user.password);
    if (!isValidPassword) return res.status(400).send({ success: false, message: "Invalid credentials" });

    const [pendingRequest] = await db.select()
      .from(pendingAdminRequests)
      .where(eq(pendingAdminRequests.userId, user.id), eq(pendingAdminRequests.status, "pending"))
      .limit(1);
    ...
    return res.status(200).send({
      ...
      user: { ...user, hasPendingAdminRequest: !!pendingRequest }
    });
...
export const signOut = async (req, res) => {
  try {
    ...
    if (userId) await deleteCachedDataByPattern(`user:${userId}*`);
...
export const getCurrentUser = async (req, res) => {
  try {
    ...
    if (cachedData) return res.status(200).send({ success: true, user: cachedData, cached: true });
    ...
    return res.status(200).send({ success: true, user: userWithoutPassword, cached: false });
...
export const updateUserProfile = async (req, res) => {
  try {
    ...
    if (Object.keys(updated).length === 0) return res.status(400).send({ success: false, message: "Nothing to update" });

    const [updatedUser] = await db.update(users).set(updated).where(eq(users.id, userId)).returning();
    const { password: _, ...userWithoutPassword } = updatedUser;

    await deleteCachedDataByPattern(`user:${userId}*`);
    await deleteCachedDataByPattern(`user:email:${updatedUser.email}*`);

    return res.status(200).send({ success: true, message: "Profile updated successfully", user: userWithoutPassword });
...
export const forgotPassword = (req, res) => res.send("forgot Password endpoint");
